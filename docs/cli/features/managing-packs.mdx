...
6. Installs verified artifacts into the pack store (`~/.local/share/nono/packs/`).
...
Installed packs live under `~/.local/share/nono/packs/<namespace>/<name>/`. Each pack directory contains the verified artifacts and a lockfile entry tracking the installed version and signer identity.
